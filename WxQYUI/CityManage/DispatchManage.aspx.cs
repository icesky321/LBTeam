...
public partial class Kefu_Info_DispatchManage : System.Web.UI.Page
{
    ...
    LB.BLL.CityManager_Config bll_citycfg = new LB.BLL.CityManager_Config();
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            if (User.Identity.IsAuthenticated)
            {
                ...
                LB.SQLServerDAL.CityManager_Config MCityCfg = new LB.SQLServerDAL.CityManager_Config();
                MCityCfg = bll_citycfg.GetCityManager_ConfigBymobileNum(userMobile);
                hfCityManagerRegionCode.Value = MCityCfg.RegionCode.ToString();
                Load_SellInfoes();
    ...
    private void Load_SellInfoes()
    {
        ...
        hfCountTodo.Value = bll_sell.GetCount_CityManagerohandle(hfCityManagerRegionCode.Value.Substring(0, 4)).ToString();
